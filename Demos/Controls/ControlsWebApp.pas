...
type
 TControlsWebAppGlobal = class(TD2BridgeServerControllerBase)
  private
...
var
 D2BridgeServerController: TControlsWebAppGlobal;


Function Controls: TControlsSession;
...
Function Controls: TControlsSession;
begin
 Result:= TControlsSession(D2BridgeInstance.PrismSession.Data);
end;

constructor TControlsWebAppGlobal.Create(AOwner: TComponent);
begin
 inherited;
 {$IFDEF D2BRIDGE} 
...
procedure TControlsWebAppGlobal.OnException(Form, Sender: TObject; E: Exception; FormName, ComponentName, EventName: string; APrismSession: IPrismSession);
begin
 //Show Error Messages
 {
...
procedure TControlsWebAppGlobal.OnNewSession(const Request: TPrismHTTPRequest; Response: TPrismHTTPResponse; Session: TPrismSession);
begin
 D2BridgeInstance.PrismSession.Data := TControlsSession.Create(Session);
...
procedure TControlsWebAppGlobal.OnCloseSession(Session: TPrismSession);
begin
...
procedure TControlsWebAppGlobal.OnExpiredSession(Session: TPrismSession; var Renew: boolean);
begin
...
procedure TControlsWebAppGlobal.OnIdleSession(Session: TPrismSession; var Renew: boolean);
begin

end;

procedure TControlsWebAppGlobal.OnDisconnectSession(Session: TPrismSession);
begin

end;

procedure TControlsWebAppGlobal.OnReconnectSession(Session: TPrismSession);
begin

end;

procedure TControlsWebAppGlobal.OnSecurity(const SecEventInfo: TSecuritEventInfo);
begin
{
...
{$IFNDEF D2BRIDGE}
initialization
 D2BridgeServerController:= TControlsWebAppGlobal.Create(D2BridgeInstance.Owner);
{$ENDIF}

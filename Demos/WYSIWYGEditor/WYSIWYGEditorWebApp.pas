...
type
 TWYSIWYGEditorWebAppGlobal = class(TD2BridgeServerControllerBase)
  private
...
var
 D2BridgeServerController: TWYSIWYGEditorWebAppGlobal;


Function WYSIWYGEditor: TWYSIWYGEditorSession;
...
Function WYSIWYGEditor: TWYSIWYGEditorSession;
begin
 Result:= TWYSIWYGEditorSession(D2BridgeInstance.PrismSession.Data);
end;

constructor TWYSIWYGEditorWebAppGlobal.Create(AOwner: TComponent);
begin
 inherited;
 {$IFDEF D2BRIDGE} 
...
procedure TWYSIWYGEditorWebAppGlobal.OnException(Form, Sender: TObject; E: Exception; FormName, ComponentName, EventName: string; APrismSession: IPrismSession);
begin
 //Show Error Messages
 {
...
procedure TWYSIWYGEditorWebAppGlobal.OnNewSession(const Request: TPrismHTTPRequest; Response: TPrismHTTPResponse; Session: TPrismSession);
begin
 D2BridgeInstance.PrismSession.Data := TWYSIWYGEditorSession.Create(Session);
...
procedure TWYSIWYGEditorWebAppGlobal.OnCloseSession(Session: TPrismSession);
begin
...
procedure TWYSIWYGEditorWebAppGlobal.OnExpiredSession(Session: TPrismSession; var Renew: boolean);
begin
...
procedure TWYSIWYGEditorWebAppGlobal.OnIdleSession(Session: TPrismSession; var Renew: boolean);
begin

end;

procedure TWYSIWYGEditorWebAppGlobal.OnDisconnectSession(Session: TPrismSession);
begin

end;

procedure TWYSIWYGEditorWebAppGlobal.OnReconnectSession(Session: TPrismSession);
begin

end;

procedure TWYSIWYGEditorWebAppGlobal.OnSecurity(const SecEventInfo: TSecuritEventInfo);
begin
{
...
{$IFNDEF D2BRIDGE}
initialization
 D2BridgeServerController:= TWYSIWYGEditorWebAppGlobal.Create(D2BridgeInstance.Owner);
{$ENDIF}

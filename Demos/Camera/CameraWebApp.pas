...
type
 TCameraWebAppGlobal = class(TD2BridgeServerControllerBase)
  private
...
var
 D2BridgeServerController: TCameraWebAppGlobal;


Function Camera: TCameraSession;
...
Function Camera: TCameraSession;
begin
 Result:= TCameraSession(D2BridgeInstance.PrismSession.Data);
end;

constructor TCameraWebAppGlobal.Create(AOwner: TComponent);
begin
 inherited;
 {$IFDEF D2BRIDGE} 
...
procedure TCameraWebAppGlobal.OnException(Form, Sender: TObject; E: Exception; FormName, ComponentName, EventName: string; APrismSession: IPrismSession);
begin
 //Show Error Messages
 {
...
procedure TCameraWebAppGlobal.OnNewSession(const Request: TPrismHTTPRequest; Response: TPrismHTTPResponse; Session: TPrismSession);
begin
 D2BridgeInstance.PrismSession.Data := TCameraSession.Create(Session);
...
procedure TCameraWebAppGlobal.OnCloseSession(Session: TPrismSession);
begin
...
procedure TCameraWebAppGlobal.OnExpiredSession(Session: TPrismSession; var Renew: boolean);
begin
...
procedure TCameraWebAppGlobal.OnIdleSession(Session: TPrismSession; var Renew: boolean);
begin

end;

procedure TCameraWebAppGlobal.OnDisconnectSession(Session: TPrismSession);
begin

end;

procedure TCameraWebAppGlobal.OnReconnectSession(Session: TPrismSession);
begin

end;

procedure TCameraWebAppGlobal.OnSecurity(const SecEventInfo: TSecuritEventInfo);
begin
{
...
{$IFNDEF D2BRIDGE}
initialization
 D2BridgeServerController:= TCameraWebAppGlobal.Create(D2BridgeInstance.Owner);
{$ENDIF}

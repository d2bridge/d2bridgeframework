...
{$IFNDEF FPC}
  ...
  if vResInfo <> 0 then
{$ELSE}
  vResInfo:= LazarusResources.Find('PRISM_Support_Security_UserAgent');
...
{$ELSE}
    vResStream := TLazarusResourceStream.Create('PRISM_Support_Security_UserAgent', nil);
{$ENDIF}
...
{$ELSE}
  if ExecRegExpr(AHeaderUserAgent, FItems[I]) then
{$ENDIF}
  begin
   Result:= true;
   break;

...
begin
 ...
 begin
  ...
  add('    <div class="input-group ' + CSSClasses + '" id="'+AnsiUpperCase(ItemPrefixID)+'" style="'+ HtmlStyle +'" '+ HtmlExtras +'>');
...
//   add('      <div class="mx-1"></div>');
 ...
 begin
  ...
  add('');
  add('      $.ajax({');
  add('        url: appBaseD2Bridge + ''/d2bridge/upload?token='+ vToken +'&formuuid='+ vFormUUID +'&prismsession='+ vUUID +'&origin=upload'+'&sender='+ Uppercase(ItemPrefixID) +''',');
  add('        type: ''POST'',');
  add('        data: formData,');
